    ...
    if (req.method === 'POST') {
      ...
      if (!leadData.fullName || !leadData.email || !leadData.company) {
        return res.status(400).json({
          success: false,
          message: 'Пожалуйста, заполните все обязательные поля (имя, email, компания)'
        })
      }
      ...
      if (process.env.ENABLE_EMAIL_NOTIFICATIONS === 'true' || process.env.ENABLE_TELEGRAM_NOTIFICATIONS === 'true') {
        Promise.allSettled([
          ...
          results.forEach((result, index) => {
            const notificationType = index === 0 ? 'Email' : 'Telegram'
            if (result.status === 'fulfilled') {
              console.log(`✅ ${notificationType} notification sent successfully`)
            } else {
              console.error(`❌ ${notificationType} notification failed:`, result.reason)
            }
          })
      ...
      return res.status(201).json({
        success: true,
        message: 'Заявка успешно отправлена! Мы свяжемся с вами в течение 48 часов. 🚀',
        data: {
          id: lead.id,
          email: lead.email,
          company: lead.company
